...
export default function LoginPage() {
  ...
  useEffect(() => {
    const roleParam = searchParams.get("role") as AccessRole | null;
    if (roleParam && (roleParam === "medecin" || roleParam === "admin")) {
  ...
  return (
    <div className="min-h-screen relative bg-zinc-950 text-zinc-50 overflow-hidden isolate">
      ...
      />
      {/* Layer 2 : ambient glow (top-center) */}
      <div
        aria-hidden
        ...
        style={{
          background:
            selectedRole === "admin"
              ? "radial-gradient(closest-side, rgba(139,92,246,0.18), transparent)"
              : "radial-gradient(closest-side, rgba(16,185,129,0.18), transparent)",
        }}
      />
      {/* Layer 3 : accent glow (bottom-right) */}
      <div
        aria-hidden
      ...
      <motion.div
        ...
        className={`pointer-events-none absolute inset-x-0 top-[22%] h-px bg-gradient-to-r from-transparent to-transparent origin-center ${
          selectedRole === "admin" ? "via-violet-500/30" : "via-emerald-500/30"
        }`}
      />
